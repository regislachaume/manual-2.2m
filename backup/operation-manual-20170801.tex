...
%\newlistof{procedure}{lop}{\listprocedurename}
\makeatletter
...
\author{%
    R.~Angeloni\\
    \texttt{rangelon@astro.puc.cl}\vspace{10pt} \\
    A.~Hempel\\
    \texttt{ang.hmpl@gmail.com}\vspace{10pt} \\
    I.~Lacerna\\
    \texttt{ialacern@astro.puc.cl}\vspace{10pt} \\
    R.~Lachaume\\
    \texttt{lachaume@astro.puc.cl}\vspace{10pt} \\
    M.~Rabus\\
    \texttt{mrabus@astro.puc.cl} 
    }
...
\def\fetchob{menu \texttt{File} $\rightarrow$ \texttt{Load OBs} $\rightarrow$ \texttt{From file…}} 
...
\begin{figure}[!ht]
\includegraphics[width=\linewidth]{dome/dome-old-control-room.jpg}
\caption{Entering the old control room in the telescope building}
\label{fig:old-control-room}
\end{figure}

\begin{figure}[!ht]
\includegraphics[width=\linewidth]{dome/dome-computer-room.jpg}
...
\begin{figure}[!ht]
\includegraphics[width=\linewidth]{dome/dome.jpg}
...
\begin{enumerate}
  \item \gls{wfi} (daily, see Fig.~\ref{fig:wfi-restart})
        \begin{enumerate}
          \item On the screen \texttt{BOB Wide Field Imager}, go to the \texttt{StartUp} virtual desktop.\label{list:wfi1}
          \item In a terminal, \texttt{osf2p2StartUp ini WFI F8}, where ini is your initials.
          \item On the dialog, choose \texttt{FULL} startup.
          \item Click \texttt{START} on the appearing window (\texttt{ESO220 WEEKLY FULL STARTUP}). 
          ...
          \item Rearrange windows to the corresponding desktops.
             ...
             \end{itemize}
          \item In \gls{bob}, execute \texttt{testBias.obd}
             \label{list:wfitest}
             \begin{itemize}
               \item Import it with \fetchob.
               \item Click the \texttt{Start} button. 
             \end{itemize}
          \item If \texttt{Cannot read "@w2p2wfi…"} pops up on the \gls{tcs} screen (Fig.~\ref{fig:wfistarterror}), click \texttt{OK}.
        \end{enumerate}
  \item \gls{tcs} (daily, see Fig.~\ref{fig:tcs-restart})
        \begin{enumerate}
  ...
  \item \gls{feros} (weekly, see Fig.~\ref{fig:feros-restart}).\\
        ...
        \begin{enumerate}
         ...
         %\item In \gls{bob}, run \texttt{StanCalnorm} (does bias, flatfield and wavelength calibration, takes about 40 min)
         %  \begin{itemize}
         %    \item Fetch it with \fetchob (\texttt{/OBD/Templates/}).
         %    \item Click \texttt{Start} (takes about 1 min).
%%%IVAN: commented since when one test the FEROS AG (point 5 below), one will probably see a saturated image due to the calibration lamp.          
           \end{itemize}
        \end{enumerate}
  \item \gls{grond} (daily)\\
        ...
        \begin{enumerate}
         ...
         \item In the case of a power cut or issues with GROND, restart the instrument.\\ 
               If GROND was unstable the night before or there was a power outage %go go it.
         \begin{enumerate}
         ...
         \item If time allows, repeat the last step with the TCS on.
           \begin{enumerate}
             ...
             \end{itemize} 
             \item Set telescope back to FEROS/WFI\\
             In a terminal type, \texttt{grondM3 WFI \&\& grondMC CLOSE \&\& grondCS CLOSE}.
           \end{enumerate}
         \item If a restart (point \ref{list:grinsstart}) was done, set the image displays.
            \begin{enumerate}
             ...
             \end{enumerate}
        \end{enumerate}
  \item Check the guide cameras on the screen \texttt{Autoguider GROND \& FEROS}.\\
        Repeat the following procedure for \gls{feros} and \gls{grond}.
        \label{list:ag}
        \begin{enumerate}
           \item Go to the \texttt{Autoguider} screen. 
           \item Rearrange windows to their corresponding virtual desktops.\\
                 \texttt{Autoguiding} window should go to all desktops.
           \item Use numbers 2, 1, 0.5 below \texttt{Autoguider control} and click \texttt{Apply}.
           \item Select the instrument below \texttt{CCD change} (see Fig.~\ref{fig:agswitch}).
           \item Use \texttt{Start exposure} to change \texttt{CCD Status} to \texttt{Infinite loop}.\\
           (Use \texttt{Stop exposure} to change \texttt{CCD Status} to \texttt{Completed} before selecting other instrument).
           \item Proceed to troubleshooting (Sect.~\ref{sec:trouble}) if \texttt{CCD Status} is not \texttt{Infinite loop}.
           \item If checking the \gls{grond} autoguider
               \begin{enumerate}
                  \item find window \texttt{Telescope R.T.D.}
                  \item use menu \texttt{TCS} $\rightarrow$ \texttt{Attach camera}
                  \item a bias image should be seen within seconds
               \end{enumerate}
           \item If checking the \gls{feros} autoguider
               \begin{enumerate}
               ...
               \end{enumerate}
           \item Set the numbers to 6, 3, 0.02.
        \end{enumerate}
        \item On the \texttt{FEROS pipeline} screen, check that their is enough free disk space
...
\subfigure[Launch the reboot from the terminal.]{
  \includegraphics[width=0.45\linewidth]{wfi/wfireboot-0.png}
}%
\hspace{0.15\linewidth}
\subfigure[Choose full startup.]{
  \includegraphics[width=0.2\linewidth]{wfi/wfireboot-1.png}
}%

\subfigure[Click start.]{
  \includegraphics[width=0.42\linewidth]{wfi/wfireboot-2.png}
  \label{fig:wfi-full-restart}
}%
\hspace{0.05\linewidth}
\subfigure[A log terminal opens. Process takes $\approx $ 10 min.]{
  \includegraphics[width=0.48\linewidth]{wfi/wfireboot-3.png}%
}
...
it.]{
  \includegraphics[width=0.41\linewidth]{wfi/wfireboot-4.png}%
}
\hspace{0.05\linewidth}
\subfigure[
Don't forget to rearrange all these windows.  
]{
  \includegraphics[width=0.47\linewidth]{wfi/wfireboot-5.png}%
}
...
\centering
\subfigure[Switch on hydraulics and drives on the \gls{windows}.]{
  \includegraphics[width=\linewidth]{dome/dome-auxfunc.png}
}
\end{minipage}
\hspace{0.15\linewidth}
\begin{minipage}{0.42\linewidth}
\subfigure[On the TCS machine, give \texttt{OK} to this error.]{
%\subfigure[Give \texttt{OK} to this error.]{
  \includegraphics[width=0.6\linewidth]{tcs/tcsreboot-initerror.png}
}
\subfigure[Reboot lte2p2 machine.]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-lccboot.png}
}
\subfigure[Launch the reboot from the terminal.]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-0.png}
}
\end{minipage}
\begin{minipage}{0.4\linewidth}
\subfigure[Click start.]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-1.png}
}%
\end{minipage}
%%%
\hspace{0.15\linewidth}
\begin{minipage}{0.45\linewidth}
  \includegraphics[width=\linewidth]{tcs/tcsreboot-2.png}\\%[.2ex]
\subfigure[Click \texttt{OK} to incoming pop-ups.]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-3.png}
}
\end{minipage}
%\hspace{0.15\linewidth}
\begin{minipage}{0.4\linewidth}
...
After clicking \texttt{OK}, telescope should move.]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-5.png}
}
\end{minipage}
\hspace{0.15\linewidth}
\begin{minipage}{0.3\linewidth}
\subfigure[Check that CCD alarms are on in the log (black terminal).]{
  \includegraphics[width=\linewidth]{tcs/tcsreboot-6.png}
}
\end{minipage}
\caption[Telescope control software restart]{\acrshort{tcs} restart. As for \acrshort{wfi} there is a black terminal
(log) and process ends with a PDF log being popped up.  Emergent windows
should be rearranged to their respective desktops.  The \acrshort{rtd} 
display should go to all desktops on the left screen, the other windows
on the right screen.}
...
is enabled in the FEROS control window.]{
  \includegraphics[width=0.66\linewidth]{feros/ferosreboot-0.png}
}\hspace{0.12\linewidth}
\subfigure[Click \texttt{FULL}.]{
  \includegraphics[width=0.18\linewidth]{feros/ferosreboot-1.png}
}\\
\subfigure[Click \texttt{START}.]{
  \includegraphics[width=0.41\linewidth]{feros/ferosreboot-2.png}
}\hspace{0.08\linewidth}%
\subfigure[This black log opens. Later on a PDF will pop-up.]{
  \includegraphics[width=0.46\linewidth]{feros/ferosreboot-3.png}
}
\subfigure[Click \texttt{START}.]{
  \includegraphics[width=0.4\linewidth]{feros/ferosreboot-4.png}
}\hspace{0.05\linewidth}
...
desktops.  \gls{rtd}, \gls{bob} and FEROS control go to \texttt{BOB+OS}.]{
  \includegraphics[width=0.5\linewidth]{feros/ferosreboot-5.png}
}
...
\section{WFI}
Internal calibrations and health checks are typically left running
in the morning after the dome has been closed.  See Sect.~\ref{sec:clocal}.

\procedure{WFI afternoon calibrations}
\begin{enumerate}
...
A linearity check is left running in the morning (see Sect.~\ref{sec:clocal}). The standard \gls{feros} calibration is generally done after startup.

\procedure{FEROS afternoon calibrations}
\begin{enumerate}
  \item Go to screen \texttt{FEROS BOB}.
  \item Execute \texttt{StanCalNorm.obd} from \gls{bob} (45 min).
    \begin{enumerate}
       \item Use \fetchob.
       \item Click \texttt{Start}.
    \end{enumerate}
  \item Check that the \gls{drs} has processed it.
    \begin{enumerate}
...
This internal calibration takes place in a separate room.  It can be run
at any time, including during a \gls{wfi} or \gls{grond} observation.

\section{GROND}

GROND calibrations are left running in the morning after the dome has
been closed.  See Sect.~\ref{sec:clocal}.
...
\section{Opening (one hour before sunset)}
\label{sec:open}

Opening should take place about one or two hours before sunset if conditions allow it. 

\procedure{Dome opening.}
\begin{enumerate}
   \label{list:open}
   \item Go to the screen \texttt{Telescope Control Software}.
   \item Ensure the main mirror cover is closed.
         \begin{enumerate}
           ...
           \item If not, click \texttt{Close}.
         \end{enumerate} 
   \item Open the slit.\label{list:slitopen}\\
         On the \gls{tcs} setup panel (Fig~\ref{fig:tcssetup}), below \texttt{slit}, click \texttt{Open}.
   \item Do these steps around sunset. 
         \begin{enumerate}
         \item If hydraulics is off, switch it on and initialize telescope (on TCS Setup Panel under \texttt{Telescope}, Fig.~\ref{fig:tcssetup}).
         \item Preset to an empty field in the East.
            ...
            \item Select field with \texttt{Up} and \texttt{Dwn}.\\
                  \textbf{Right ascension should be about 1 hour more than sidereal time at sunset.}\\
         ...
         On the \gls{tcs} setup panel (Fig~\ref{fig:tcssetup}), below
         \texttt{Dome},
         select the checkbox \texttt{Automatic}.
       \item Open the mirror cover.\label{list:mirropen}\\
         On the  \gls{tcs} setup panel (Fig~\ref{fig:tcssetup}), below \texttt{Main Mirror Cover}, click \texttt{Open}.         
...
\begin{enumerate}
   \item If hydraulics is off, switch it on and initialise telescope.
   \item Activate the connection between instruments and telescopes
       \begin{enumerate}
         \item On the \gls{feros} control panel (Fig.~\ref{fig:feroscon}), use menu \texttt{Telescope} $\rightarrow$ \texttt{Enable}.
         ...
         \item Refresh the \gls{wfi} general state panel (Fig.~\ref{fig:wfigen}).
               \begin{itemize}
                 \item Use menu \texttt{Std. Options} $\rightarrow$ \texttt{Refresh Database values}.
...
\begin{enumerate}
  ...
  \item If not doing the standard filtres, customise \gls{ob}.
    \begin{enumerate}
       \item For non standard filtres, edit the filter names.
             \label{list:editfilt} 
              \begin{enumerate}
...
Pointing with \gls{wfi} is important even if the
instrument is not used; for \gls{grond} and \gls{feros} use it to define their
pointing model.  If things go well, the bright star that is pointed at should
fall a few tens of pixels from the centre of the mozaic in the South-East
direction, on pixel (4150,3950). The telescope pointing has an rms accuracy of better than 10 $-$ 20 arcsecs (4 pixels correspond roughly to 1 arcsec).
If not, ensure that point~\ref{list:switchwfi} has been done properly 
...
\begin{enumerate}
  ...
  %(really that large, I try for 0.5arcsec)\\
...
Before taking evening flat fields, check the relevant items of ``no flux or
little flux'' in Sect.~\ref{sec:noflux}. In particular the main mirror cover
should be open and the dome set in automatic.  The sky flats typically start when
the sun is 4 degrees below the horizon.  To be able to start on time, the
following procedure should be started right at sunset.
...
\begin{enumerate}
  ...
  \item Set mirrors and open shutters
     \begin{enumerate}
     ...
     \end{enumerate}
  \item Circumvent possible errors (if not done at sunset)
     \begin{enumerate}
       \item Close \gls{bob}.
       \item In the terminal, type \texttt{grondSHUTTER}.
  ...
  \item Get sky flat \gls{ob} ready\\
       %\begin{enumerate}
         ...
         %\item Deactivate preset by right-clicking on the triangle of the
         %      acquisition template.\\
          %     (A thumb down should appear.  If not, go on clicking until it appears.) 
       %\end{enumerate}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Use menu \texttt{Instrument selection} $\rightarrow$ \texttt{WFI}
  \end{enumerate}
...
overheads by parallelising preset and mirror movements.

\procedure{Switch to FEROS observation}
\begin{enumerate}
  \item Switch the pointing model to \gls{feros}.\label{list:pmferos}
  \begin{enumerate}
    \item Go to screen \texttt{Telescope Control Software}.
    \item Go to the \texttt{TCS Status Panel} (workspace \texttt{Status}, see Fig.~\ref{fig:tcsstatus}). 
    \item Use menu \texttt{Instrument selection} $\rightarrow$ \texttt{FEROS}
  \end{enumerate}
  \item Check the \gls{grond} M3 mirror.\\
        Go to screen \texttt{GROND BOB}.
  \begin{enumerate}
      \item Ensure that the mirror is on \gls{wfi}.\label{list:feros:checkm3}
      \begin{enumerate}
         \item Go to the \texttt{GROND Control} panel (workspace \texttt{BOB+OS}).
         \item Check the value of \texttt{GROND M3}.
      \end{enumerate}
      \item If it is on \gls{grond}, set it to \gls{wfi}.\\
            In a terminal, type \texttt{grondM3 WFI}.\\
            Redo point \ref{list:feros:checkm3} (value should be MOVING then WFI).
  \end{enumerate}
  \item Check the \gls{feros} \texttt{mirr3} mirror.\\
        Go to screen \texttt{FEROS BOB}.
  \begin{enumerate}
      \item Ensure that the mirror is on \gls{feros}
      \begin{enumerate}
         \item Go to the \texttt{FEROS Control} panel (workspace \texttt{BOB+OS}).
         \item Check the value of \texttt{M3 Selected Mirror}.
      \end{enumerate}
...
You can start an OB once point \ref{list:pmgrond} has been done.  This minimises
overheads by parallelising preset and mirror movements.  If the Moon is between the first target and the current position, you should manually preset with the main cover closed.

\procedure{Switch to GROND observations}
\begin{enumerate}
  \item \textbf{Important (security): } Ensure the \gls{grond} main cover is closed.
        \begin{enumerate}
          \item Go to screen \texttt{GROND BOB}.
          \item In a terminal, type \texttt{grondMC CLOSE}.
          \item Wait for cover to close (10-15 s).
            \begin{itemize}
              \item Check value on panel \texttt{GROND Control} (Moving, then Closed).
            \end{itemize}
        \end{enumerate}
  \item Switch the pointing model to \gls{grond}.\label{list:pmgrond}
  \begin{enumerate}
    \item Go to screen \texttt{Telescope Control Software}.
    \item Go to the \texttt{TCS Status Panel} (workspace \texttt{Status}, see Fig.~\ref{fig:tcsstatus}). 
    \item Use menu \texttt{Instrument selection} $\rightarrow$ \texttt{GROND}
  \end{enumerate}
  \item Check the \gls{grond} M3 mirror.\\
        Go to screen \texttt{GROND BOB}.
  \begin{enumerate}
      \item Ensure that the mirror is on \gls{grond}.\label{list:grond:checkm3}
      \begin{enumerate}
         \item Go to the \texttt{GROND Control} panel (workspace \texttt{BOB+OS}).
         \item Check the value of \texttt{GROND M3}.
      \end{enumerate}
...
\begin{enumerate}
  \item Check that your exposure is beeing read out automatically, if not,
     do it manually
    \begin{itemize}
...
\begin{enumerate}
\item Switch on the light in the dome to monitor the following on the \gls{webcam} (Fig~\ref{fig:webcam}).\\ 
...
\procedure{Put the instrument offline.}
\begin{enumerate}
\item Close the \gls{wfi} protective shutter.\\
...
Turn off ventilation and hydraulics using the \gls{domefunc} (\gls{windows}, Fig.~\ref{fig:domefunc}). If it is required, use the password on left side of the screen.
\begin{enumerate}
  ...
  \item Turn off the drives.\\
        Click \texttt{DrivesOff}. The button above \texttt{Drives} should go red.
\end{enumerate}

\section{Tidying folders}

Night data are moved to a night directory and disk space is checked.
...
\begin{enumerate}
  ...
  \item On the \gls{grond} workstation, <check it with GROND team>.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textit{(\gls{grond})} If images look like biases, restart FIERA.\\
...
\begin{enumerate}
  ...
  \item \gls{grond}
    \begin{enumerate}
       ...
       \begin{enumerate}
          \item If a FIERA exposure is running in \texttt{GROND Control}, click \texttt{End} or let it finish.
...
\label{sec:FEROSDRSrestart}
  \begin{itemize} 
    \item Stop Data Subscriber and \gls{drs} at the \gls{drs} computer (Fig.~\ref{fig:datasubscriber}).
    ...
    \item Then restart the Data Subscriber and FEROS DRS:
    \begin{itemize}
        \item Left click start Data Subscriber
        \item Configure Data Subscriber (Fig.~\ref{fig:datasubconfig}).
            \begin{itemize}
                \item \texttt{Program ID: service}
                \item \texttt{Observer Name: service}
                \item \texttt{Rename to Keyword: Name on INS ws}
             \end{itemize}
        \item Left click start \texttt{FEROS DRS}, start from top to bottom
            \begin{itemize}
...
\subsection{Start-Up}
\begin{itemize}
\item If the Start-Up stalls at lccBoot, e.g. for l2p2cam: in a xterm window >rlogin l2p2cam > reboot , resets the connection to the camera directly, not via the Start-Up procedure, wait a few minutes and then press CONTINUE in the Start/Shutdown panel
\end{itemize}
...
\begin{itemize}
 \item Type \texttt{e2p2NewShutDown} and wait for it to complete.
 \item Type \texttt{e2p2NewStartUp} and wait for it to complete.
 \item On the \gls{tcs} setup panel (Fig~\ref{fig:tcssetup}) press \texttt{Initialize} under \texttt{Telescope}.
\end{itemize}
...
\begin{itemize}
  \item Identify the process id of the panel\\
        Type \texttt{ps aux | grep tcsSetup} in a terminal.
  \item See which process depends on it\\
        Type \texttt{pstree -a <process id>}
  \item If process \texttt{rs232} (dome controls) is listed, kill it.\\
        Type \texttt{ps aux | grep rs232}\\
        Type \texttt{kill <process id>}
\end{itemize}
...
\caption[The GROND IRACE box]{The GROND infrared electronics (IRACE) box below the main mirror of the telescope.  The power switch
...
the OB stalls. If \texttt{grondFM} doesn't solve the issue, this may need a full reset of the IRACE (IR electronics).
...
\begin{enumerate}
  ...
  \item Locate the IRACE workstation (Fig.~\ref{fig:computer-room}) and shut it down.
  \item Go to the dome 
  \item Locate the power switch of the IRACE box (Fig.~\ref{fig:grond-irace}) below the main mirror.
...
\begin{enumerate}
  ...
  \item After a few minutes refresh \texttt{http://www.ls.eso.org/lasilla/sciops/2p2/EnvMon}.
\end{enumerate}
