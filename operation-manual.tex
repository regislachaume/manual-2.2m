...
\newglossaryentry{uws2p2}{name=\texttt{uws2p2},sort=uws2p2,description={User worksation of the 2.2m metre telescope. All workstation are virtually run on it.}}
...
\newglossaryentry{preset}{name=preset,description={Telescope movements (right ascensio, declination, and secondary mirror focusing) needed to point at a new source. It is also used as a verb to refer to the action of ordering the telescope to move.}}
...
%%%%
  \item \label{list:wfistartup}\textbf{\gls{wfi} restart (8 min, in case of problems, see \figref{wfi-restart})}
        \begin{enumerate}
          ...
          \item Rearrange windows to the corresponding desktops (1 min, see \figref{wfisu-windows})
             \label{list:wfiwin}
             \begin{itemize}
...
\item Prepare the telescope
	...
	\item\label{list:mirropen}Open the main mirror cover
            ...
            \end{enumerate}
        \end{enumerate}
        \item \Gls{preset} to an empty field if doing sky flats, a test with GROND, or readying before sunset
...
\begin{enumerate}
  ...
  \item Check for mirrors
     ...
     \begin{enumerate}
        \item Check \gls{feros} mirror (\texttt{M3 Selected Mirror}) on the \gls{feros} control panel (\figref{feroscon}).\\
              If necessary, set \texttt{mirr3} motor on the \gls{ics} (\figref{ferosics})
              \begin{itemize}
...
\begin{enumerate}
  \item If not done, check that dome and main mirror cover are open (\procref{open}). 
  \item If not done, check \gls{grond} and \gls{feros} mirrors (\procref{insready}, point~\ref{list:checkmirror}).
  \item If not done, check that the WFI main cover is open (\procref{insready}, point~\ref{list:opencov}).
  ...
  \item Execute \gls{ob}
    \begin{enumerate}
      ...
      \item For each template
       \begin{enumerate}
         \item Click \texttt{OK} to pop-up asking to \gls{preset}.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Go to the \texttt{TCS Status Panel} (workspace \texttt{Status}, see \figref{tcsstatus}).
  ...
  \end{enumerate}
  \item Point at a FEROS focus/standard star. See \secref{ferosfocus}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item If not done, check \gls{grond} and \gls{feros} mirrors (\secref{insprep}, point~\ref{list:checkmirror}).
  \item If not done, check that the WFI main cover is open (\secref{insprep}, point~\ref{list:opencov})\label{list:pointing:opencov}.
  \item Go to \gls{bob} on screen \texttt{BOB Wide Field Imager}.
...
\begin{enumerate}
  \item Check that dome and main mirror are open (\procref{open}).\label{list:ferosfocus1}
  \item Check \gls{grond} and \gls{feros} mirrors (on \gls{wfi} and \gls{feros}, respectively. \secref{insprep}, point~\ref{list:checkmirror}).
  \item Check that the WFI main cover is open (\secref{insprep}, point~\ref{list:opencov}).
...
In the GROND Control panel (\figref{grondcon}), click \texttt{RRM ONLINE} at the end of the twilight (beginning of the night) to allow for remote observations with this instrument.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Go to screen \texttt{Telescope Control Software}.
        \item Locate the \texttt{TCS Control Panel} (\figref{tcs}).
  ...
  \end{enumerate}
  \item On the \texttt{TCS Control Panel} (\figref{tcs}) start the guiding
  \begin{enumerate}
...
\begin{enumerate}
    ...
    \item On the \texttt{TCS Control Panel} (\figref{tcs})
    \begin{enumerate}
...
when starting an OB. For observations requiring guiding, you should make sure
that it is already on or acquire it right after the first exposure begins.  On
\gls{wfi}, it should be avoided if the observing template sets \texttt{RETURN}
to F (false) and pointing position is important.
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item In \gls{bob}, deactivate the \gls{preset} template\\
...
\item Close the main mirror cover.\\ 
      In the \texttt{TCS Setup Panel} (\figref{tcssetup}), click \texttt{Close} below \texttt{Main Mirror Cover}. 
...
\begin{enumerate}
\item Close the \gls{wfi} protective shutter.\\
      Check on the General State panel in the WFI monitor (\figref{wfigen}) that \texttt{Protective Shutter State} is \texttt{CLOSED}.\\
...
\item Deactivate \gls{grond} communication.\\ 
      In the GROND Control panel (\figref{grondcon}), click \texttt{RRM STANDBY} and \texttt{TCS OFF}
...
\item Ensure \gls{feros} mirror is not in the way.\\
      In panel FEROS Control (\figref{feroscon}), \texttt{M3 Selection Mirror} should state \texttt{WFI}\\
      If not, int the \gls{ics} control panel (\figref{ferosics}), check the \texttt{mirr3} box, select \texttt{WFI}, click \texttt{SETUP}.
\item Ensure \gls{grond} mirror is not in the way.\\
      In panel  GROND Control (\figref{grondcon}), \texttt{GROND M3} should state \texttt{WFI}.\\
...
\begin{itemize}
  \item the slit is \texttt{OPEN} (\gls{tcs} setup, \figref{tcssetup}).
  ...
  \item \textit{(\gls{grond})} the protective and cold shutters are \texttt{OPEN} (\gls{grond} control,
        \figref{grondcon}).  Use \texttt{grondMC OPEN} and \texttt{grondCS OPEN} in the terminal if necessary.
...
\subfigure[\text{\gls{l2p2cam} power cycle}]{%
...
\begin{enumerate}
    ...
    \item On the \texttt{TCS Status} panel locate the modules section (\figref{tcsstatus})
...
\subsection{GROND}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see \procref{agmodules}.  You will need to uncheck them just before starting the guider software with \texttt{e2p2StartTCCDs}.
...
\subsection{FEROS}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see \procref{agmodules}.  You will need to uncheck them before starting the guider software with \texttt{e2p2StartTCCDs} (step~\ref{lab:fag2}). 
...
It may happen that an OB stalls, that means, nothing is happening.  No error is reported but the telescope does not move, no integration seems to take place and the \gls{bob} windows has stopped to give information about what is going on.
...
set it manually.\\
In the TCS screen, enter focus value in the main panel (\figref{tcs}).\\
  ...
  \item \gls{wfi}
    \begin{enumerate}
       \item \gls{wfi} stalls just before the read out of an exposure.\\
             Abort it manually, if unsuccessfull restart \gls{fiera} (follow point \ref{list:wfiexpocrash} in \secref{expocrash})
    \end{enumerate}
  \item \gls{grond}
    \begin{enumerate}
       \item \gls{ob} crashes with ``error closing cam environment''
...
If the OB crashes at the moment to change the filtre using \gls{wfi} and the \texttt{Filter Name} status is \texttt{Moving} on the WFI General State panel (\figref{wfigen})
 \begin{enumerate}
...
\item If repeated red messages concerning M2 preset appear below 
 \texttt{VME messages} on the
  \gls{tcs} main panel (\figref{tcs}), break the infinite set-focus loop
  \begin{itemize} 
...
If it goes past 10\,000, the reduction software will fail.  It is very easy to fixe before it has reached the limit.
...
If you cannot start the \gls{feros} \gls{drs} this might be for the following reason: when rebooting \gls{w2p2off} without bringing down the pipline before, a hidden lock file may survive. If all other attempts have failed to start the DRS, follow these steps.
...
\begin{enumerate} 
    \item Go to screen \texttt{\gls{w2p2pl} Pipline}
...
\begin{itemize}
 ...
 \item There is a problem with the FEROS telemetry at TCS (window should show some graphs, \figref{tcstelemetry}).\\
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Locate the VME (\figref{computer-room}) in front of the entrance, slightly to the left.
...
A quick restart the \gls{tcs} (\secref{tcsrestart}) is generally needed right after a reboot of the VME.
...
\begin{itemize}
  \item \textit{(\gls{wfi})} OB stalls when focus order is sent by \gls{bob}.\\
        Set it manually from the \gls{tcs} main panel (\figref{tcs})
...
\begin{enumerate}
   \item Go to the dome
   \item Locate the gray 1.5-metre-high ADAM contol box (\figref{dome}).
...
\begin{itemize}
  ...
  \item Reset the mirror from the dome.
    \begin{enumerate}
       \item In the dome, locate the M3/MC box below the main mirror (\figref{dome}).
       \item On the box (\figref{grondM3-box}), press the grondM3 WFI button.
    \end{enumerate}
\end{itemize}
...
\begin{enumerate}
  ...
  \item Go to the dome 
  \item Locate the power switch of the \gls{irace} box (\figref{grond-irace}) below the main mirror.
...
\begin{enumerate}   
    \item If not done, check the right instrument is selected on the \gls{tcs} \texttt{Status Panel}
        (\figref{tcsstatus}).
    \item Check if the current Pointing Model parameters in Table.~\ref{tab:pointingmodelcoeff} for \gls{wfi} are selected on the \gls{tcs} Setup Panel (blue numbers in the bottom right-hand panel of \figref{tcssetup}). Typically
...
\begin{enumerate}   
    ...
    \item Locate the VME rack (\figref{computer-room}).
    \item On the VME, set the switch away from d (\figref{tel-remote}).
    \item Go to the dome and find the joystick (\figref{tel-joystick}).
