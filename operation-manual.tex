...
\newglossaryentry{ferosag}{name=\texttt{E2P2 Real Time Display},sort=E2P2 REAL TIME DISPLAY,description={Window with the mage of the \gls{feros} \acrlong{ag} camera}}
\newglossaryentry{wfiics}{name=\texttt{e2p2 OS GUI},sort=e2p2 OS GUI,description={\gls{wfi} \acrlong{ics} panel}}
\newglossaryentry{ferosics}{name={FEROS \texttt{ICS}},sort=FEROS ICS,description={\gls{feros} \acrlong{ics} panel}}
...
\procedure{Focusing with WFI (outline)}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
    \end{enumerate}
    \item Start the focus \gls{ob} and perform \gls{acquisition} if required
    \begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item If the fit and optimum focus offset value are correct, click \texttt{OK} (or Yes?)
            \item If the fit is not correct but you can clearly see the best value, clikc{No} and enter value in the next pop-up
            \item If you are not sure, click \texttt{Redo} and pick another star sequence
            \item If it is clear that conditions have messed the focus, click \texttt{Abort}.
        \end{itemize}
        \item Wait for a pop-up to ask whether to log the value (20 s?)
        \begin{itemize}
            \item If you are happy with the focus, click \texttt{Log}
            \item Otherwise, click \texttt{Do not log}
        \end{itemize}
        \item Wait for OB to finish (20 s?) 
    \end{enumerate}
\end{enumerate}
...
before you go to the \gls{midas} window (in the \texttt{MIDAS} \gls{desktop}), then left-click and right-click on the upper star in whatever vertical sequence to measure \gls{focus}.  
...
\begin{enumerate}
  ...
  \begin{enumerate}
     ...
     \item Shortly after, a pop-up asks to select a star to \gls{focus} on
        \begin{enumerate}
           \item A \gls{midas} image appears, left and right-click on a non-saturated source. 
...
\begin{enumerate}
  ...
  In window \gls{ferosag}, click \texttt{Centering} below \texttt{Image Control} and click on the target. 
  \item In \gls{autoguiding} window, choose integration times (6, 3, $x$ for bright targets with $x < 3$).
...
\begin{enumerate}
  ...
  \item \textbf{Note:} when observing with the \gls{adc}, don't lose time centering on the first pop-up.  No second
...
\begin{enumerate}
  ...
  \item In \gls{autoguiding} window, click \texttt{Start guiding}.
\end{enumerate}
