        ...
        \begin{enumerate}
          ...
          \item Wait for the filtre wheel to be set to a specific filtre (1--2 min)\label{list:wfisu-filtre}
                \begin{enumerate}
                    ...
                    \item If it stays in \texttt{MOVING} for more than 2 minutes or you skipped full restart, reset filtre wheel \procrefp{filtrereset}. 
                \end{enumerate}
...
\begin{enumerate}
  \item If not done, check that dome and main mirror cover are open (\secref{open}, point~\ref{list:open}). 
  \item If not done, check \gls{grond} and \gls{feros} mirrors (\secref{insprep}, point~\ref{list:checkmirror}).
  \item If not done, check that the WFI main cover is open (\secref{insprep}, point~\ref{list:opencov}).
  \item If not done, point the telescope to an empty field (\secref{open}, point~\ref{list:presetempty}).
      \begin{enumerate}
...
\begin{enumerate}
  \item Check that dome and main mirror are open (\secref{open}, point~\ref{list:open}).\label{list:ferosfocus1}
...
\begin{enumerate}
  \item If not done, check that dome and main mirror are open (\secref{open}, point~\ref{list:open}). 
  \item If not done, point the telescope to an empty field (\secref{open}, point~\ref{list:presetempty}).
  \item If not done, set the dome in automatic.
  \item Go to \texttt{GROND BOB} screen.
  \item If not done in the evening, do some prophylaxis (\secref{startup}, points~\ref{list:grond-prophylaxis} and \ref{list:grond-testtcs})
     \begin{enumerate}
       \item Close \gls{bob}.
...
\label{proc:FEROSDRSrestart}
  \begin{enumerate} 
    \item \label{list:stopDRS}Stop Data Subscriber and \gls{drs} at the \gls{drs} computer (\figref{datasubscriber}).
      \begin{enumerate}
    ...
    \begin{enumerate}
        \item \label{list:DS} Left click start Data Subscriber and configure it  (\figref{datasubconfig}).
            \begin{enumerate}
