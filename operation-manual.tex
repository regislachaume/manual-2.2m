...
\makeatother
\author{%
    R.~Angeloni\vspace{10pt}\\
    P.~Eigenthaler\\
    \texttt{paul.eigenthaler@icloud.com}\vspace{10pt}\\
    A.~Hempel\\
    \texttt{ang.hmpl@gmail.com}\vspace{10pt} \\
    I.~Lacerna\vspace{10pt}\\
    R.~Lachaume\\
...
%%%% WFI
%%%%
  \item \textbf{\gls{wfi} restart (14 min, in case of problems, see Fig.~\ref{fig:wfi-restart})}\\
        If no problem occurred during the last night, try steps \ref{list:wfisu-filtre}--\ref{list:wfitest}.  If an issue happens, do the full start-up.
        \begin{enumerate}
          ...
          \item On the dialog, choose \texttt{FULL} startup (Fig.~\ref{fig:wfisu-full}).
          ...
          \item Rearrange windows to the corresponding desktops (1 min, see Fig.~\ref{fig:wfisu-windows})
            ...
            \end{itemize}
          \item Wait for the filter wheel to be set to a specific filter (1--2 min)\label{list:wfisu-filtre}
                \begin{itemize}
         ...
         \begin{enumerate}
            ...
            \item If hydraulics were off, initialise telescope
                \begin{enumerate}
                    ...
                    \item Wait for the telescope status of  the \texttt{Telescope Control} to go from \texttt{WaitIni} to \texttt{Slew} (2 min)
                \end{enumerate}
            \item Open the main mirror cover
            \begin{enumerate}
               \item Click \texttt{Open} under the \texttt{Main Mirror Cover}.
               \item Wait for opening to complete (2 min) befor starting the flat-field \gls{ob}
            \end{enumerate}
...
After a filter change or when running several observing blocks on the same
...
\subsection{GROND}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see Procedure~\ref{proc:agmodules}.  You will need to uncheck them just before starting the guider software. 
...
\subsection{FEROS}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see Procedure~\ref{proc:agmodules}.  You will need to uncheck them before starting the guider software (step~\ref{lab:fag2}). 

\procedure{Restart the FEROS autoguider}
\label{proc:restartfag}
\begin{enumerate}
    \item Restart the \gls{ag} software.
        \begin{enumerate}
        ...
        \end{enumerate}
    \item If the problem persists or an error pops up in step~\ref{lab:fag2}, reboot the \gls{ag} \gls{lcu}.     
        \begin{enumerate}
        ...
        \end{enumerate}
    \item If the problem persists or an error pops up in step~\ref{lab:fag2}, perform a hardware reset of the \gls{lcu}.
    \begin{enumerate}
  ...
  \item \gls{grond}
    \begin{enumerate}
       ...
       \begin{enumerate}
            ...
            \item If it fails, you may need $\approx 40$\,min to deep restart (Procedure~\ref{proc:iracerestart}) IRACE.
       \end{enumerate}
...
\procedure{Help WFI to get ONLINE}
\begin{enumerate}
  \item Ensure that the error comes from electronics configuration not being read\\
	Locate or open a \texttt{logMonitor}\\
	If it was not active, choose to inspect the last 500 lines\\	
	Find red error ``Failed to dispatch command for camera LOADED OK''.\\
	Lines below and above contain ``C40 ping failed'' and ``Could not download FIERA config''
  \item If it is the case, go to the telescope floor.
  \item Power cycle the WFI electronics located at the bottom of the telescope.
  \item Do a full WFI restart
  \item If the error happens again, reboot the FIERA workstation followed by a full restart\\
	In a terminal, type \texttt{rsh -l fcdrun wwffcd reboot}, wait 2--3 minutes, then restart WFI.
\end{enumerate}
    ...
    \item Manually initialise the telescope
        \begin{enumerate}
            \item\label{lab:notinit1} 
                In a terminal type \texttt{\textasciitilde/bin/telinit}
