...
\subsection{Panels and terminals won't open}
If terminals and panels do not open on several screens (e.g. TCS and WFI), there is a problem with the user workstation \gls{uws2p2}, namely that there are to many defunct \texttt{rsh}/\texttt{remsh} connections from the user WS to the other machines.  

The easiest solution is to reboot the user workstation (Procedure~\ref{proc:uwsreboot}), which will require a full restart, but about one hour will be lost.  

Alternatively, old \texttt{rsh} and/or \texttt{remsh} from the user workstation to \gls{w2p2ins}, \gls{wferos}, \gls{wgrond}, \gls{w2p2tcs} can be killed (use \texttt{kill -9 <pid>}), but your mileage may vary...

A regular prophylactic reboot of \gls{uws2p2} once a semester should avoid this issue from occurring at night time.
