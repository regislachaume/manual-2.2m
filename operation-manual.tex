...
\newglossaryentry{ot}{name=\texttt{ot},sort=ot,description={observing tool, the application that can be used to manage the observing blocks for service observations}}
...
\newglossaryentry{rose diagram}{name=\texttt{rose diagram},sort=rose diagram,description={Diagram of the telescope position, slit orientation, and Moon displayed on the Telescope Control Panel.}}
...
\newglossaryentry{p2pp}{name=\texttt{p2pp},sort=p2pp,description={phase 2 preparation tool, the application used by observers and PIs to create their observing blocks and execute them on the mountain.}}
\newglossaryentry{bob}{name=\texttt{bob},sort=bob,description={broker for observing blocks, the application that reads observing blocks and sends commands to the telescope and instrument so that observation is performed. The \texttt{bob} panel is usually placed in the first virtual desktop \texttt{bob+gen. state}}}
...
\newglossaryentry{TCCD}{name=TCCD,description={A technical CCD a used for guiding and, in the case of FEROS, acquisition. The name pops up in the command line when restarting autoguiding cameras, e.g. \texttt{e2p2StartTCCDs}}}
\newglossaryentry{SCCD}{name=SCCD,description={Science CCD.  The name pops up in the command line when restarting an instrument step by step, e.g. \texttt{wfinsStartTCCDS}}}
\newglossaryentry{template}{name=template,description={Atomic part of an observing block, with a single function, such as the acquisition or a set of integrations with a single instrument configuration (filtre, readout mode).}}
...
\newglossaryentry{vme}{name=VME,description={virtual machine environment, used metonymously to describe the logical control unit (see LCU) controlling the telescope. (VME describes the operating system running on logical control units, but this manual will not use this definition.)}}
\newglossaryentry{flip mirror}{name=flip mirror,description={Flip mirror, mirror in the infrared arm of GROND that allows fast dithering while optical is exposing. See grondFM}}
...
\newglossaryentry{grondGRI}{name=\texttt{grondGRI},sort=grondGRI,description={Command to reinitialise the events server of GROND}}
...
\newglossaryentry{wwffcd}{name=\texttt{wwffcd},sort=wwffcd,description={Workstation controlling the WFI detector electronics (FIERA)}}
\newglossaryentry{wfefcd}{name=\texttt{wfefcd},sort=wfefcd,description={Workstation controlling the FEROS detector electronics (FIERA)}}
\newglossaryentry{wgrccd}{name=\texttt{wgrccd},sort=wgrccd,description={Workstation controlling the GROND optical detector electronics (FIERA)}}
...
\newglossaryentry{M3}{name=M3,description={Tertiary mirror. There are two tertiary mirrors, one for GROND (trumps FEROS and WFI) and one for FEROS (trumps WFI). WFI is at the secondary focus and gets the light if neither of the tertiaries is inserted in the optical path.}}
...
\newglossaryentry{wgrdcs}{name=\texttt{wgrdcs},sort=wgrdcs,description={Workstation controlling the GROND infrared detector electronics (FIERA)}}
\newglossaryentry{w2p2dhs}{name=\texttt{w2p2dhs},sort=w2p2dhs,description={Workstation controlling the observing blocks}}
\newglossaryentry{w2p2off}{name=\texttt{w2p2off},sort=w2p2off,description={Workstation controlling the reduction of FEROS data}}
...
Support astronomers perform or help to perform observations in service mode for the follow-up of gamma-ray bursts and transients (15\% of time as of 2019) and time-domain programmes (15\% of time as of 2019). They ensure that visitors correctly share their time with these other programmes.  In addition, during \gls{mpia} time, they also have they monitor the different MPIA programmes and ensure they are observed according to their priority and weather conditions. It includes real-time assessment and decisions, guided by the instructions of the different PIs, in particular during service mode.  During \gls{mpe} and Chilean time, they are mostly supporting observations and are not expecting to take real-time decisions concerning these programmes.  If service mode for MPE or Chile is offered, it is more similar to a ``delegated visitor'' mode where instructions by the PI are expected.

When no service astronomer is present (e.g. last part of the night), visiting astronomers will be asked to perform service observations for other groups, both for the follow-up of gamma-ray bursts and time-domain programmes.  The number of allocated nights for visitors takes the average time loss into account, in case more down time, compensation will be considered in service mode.  Since 2017, a long-term monitoring of quasi-stellar objects takes 1.5 to 2 hours daily, and targets of opportunity about 1 hour (15\% long-term average).  \gls{mpia} observers may be ask for additional service observations for the MPIA.
...
%%%%
  \item \label{list:wfistartup}\textbf{\gls{wfi} restart (8 min, in case of problems, see \figref{wfi-restart})}
        \begin{enumerate}
          ...
          \item In a terminal, \texttt{osf2p2StartUp ini WFI F8}, where ini is your initials (\figref{wfisu-launch}).
  ...
  \item \textbf{\gls{tcs} restart (18 min, daily, see \figref{tcs-restart})}
        ...
        \item\label{lab:hydron} Switch hydraulics and drives on
          ...
          \begin{enumerate}
                ...
                \item On the \gls{skycat} window, use \wmenu{TCS}{Pick Reference Star...}
        ...
        \begin{enumerate}
         ...
         \item Do some preventive reinitialisations (1.5 min)\label{list:grond-prophylaxis}
           \begin{enumerate}
             ...
             \item In the same terminal, execute \texttt{\gls{grondGRI}}.
  ...
  \item \textbf{\gls{feros} and \gls{grond} guide cameras setup} (3--8 min)\label{list:ag}
        \begin{enumerate}
           ...
           \item Rearrange windows to their corresponding \gls{desktop}s (1 min)\\
                 \texttt{Autoguiding} window should go to all \gls{desktop}s.
...
window of \figref{wfi-full-restart}. This PDF log opens. You can close 
it.]{
...
\gls{desktop}s.  \gls{rtd}, \gls{bob} and FEROS control go to \texttt{BOB+OS}.]{
   ...
   \begin{enumerate}
     ...
     \begin{enumerate}
        ...
        \begin{itemize}
           ...
           \item During an observing period or a visitor run, a custom \gls{ob} may be used\\
  ...
  \item Check for mirrors
     ...
     \begin{enumerate}
        \item Check \gls{feros} mirror (\texttt{\gls{M3} Selected Mirror}) on the \gls{feros} control panel (\figref{feroscon}).\\
              If necessary, set \texttt{\gls{mirr3}} motor on the \gls{ics} (\figref{ferosics})
              \begin{itemize}
       ...
       \item For non standard filtres, edit the filtre names.
             \label{list:editfilt} 
              \begin{enumerate}
                 ...
                 \item Use \wmenu{Interface}{Engineering}.
                 \item Middle-click the filtre name, fill in value, and type enter.\\
                      (In \gls{desktop} \texttt{OS GUI}, a filtre list is found below \texttt{SETUP Instrument})
              \end{enumerate}
...
Try to guide. If guiding is unstable, better to have it off.

When \gls{focus} sequence has been taken ($\approx 6$ min), click \texttt{OK}
before you go to the MIDAS window (in the Midas \gls{desktop}), then left-click and right-click on the upper star in whatever vertical sequence to measure \gls{focus}.  

Many times, you cannot adjust the measuring box with the up and down keys, so the \gls{focus} is badly estimated (the fits do not have a parabolic shape).  In that case, choose to remeasure it and the box should become adjustable (e.g. choose the upper star on other sequence).
...
\begin{enumerate}
  ...
  \item Check \gls{grond} \gls{M3} and \gls{feros} \gls{mirr3} mirrors (on \gls{wfi} and \gls{feros}, respectively. \secref{insprep}, point~\ref{list:checkmirror}).
  ...
  \item Got to \gls{bob} on screen \texttt{FEROS \gls{ob}} and fetch \gls{focus} \gls{ob}:
  \begin{enumerate}
     \item Use \fetchob.
  ...
  \begin{enumerate}
     ...
     \item A pop-up asks and suggest a \gls{focus} estimate
       ...
       \end{enumerate}
     \item Shortly after, a pop-up asks to select a star to \gls{focus} on
        \begin{enumerate}
           \item A \texttt{MIDAS} image appears, left and right-click on a non-saturated source. 
           \item Click \texttt{OK} to pop-up
        \end{enumerate}
     \item After about 5--10 min a fit to \gls{focus} is done and a pop-up asks whether it is correct
        \begin{enumerate}
...
\item Perform steps of \secref{ferosfocus} except that
   ...
   \end{itemize}
\item After \gls{focus} is done, change integration time to 0.01 on the \texttt{Autoguiding} window.
  ...
  \item Monitor the $J$-band counts.
        \begin{enumerate}
           ...
           \item Draw cuts in the infrared image.
              ...
              \end{itemize}
           \item Wait for the cuts to look flat and with about 20\,000 counts. This should occur when the sun is about 4 degrees below the horizon.
        \end{enumerate}
...
guiding will be asked at the beginning of each exposure.  It will also be asked for if at the beginning of a new \gls{template} if the filtre is changed.
...
\begin{enumerate}
  ...
  \begin{enumerate}
      ...
      \begin{enumerate}
         \item Go to the \texttt{GROND Control} panel (\gls{desktop} \texttt{BOB+OS}).
  ...
  \begin{enumerate}
      ...
      \begin{enumerate}
         \item Go to the \texttt{FEROS Control} panel (\gls{desktop} \texttt{BOB+OS}).
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item On the autoguider windows (\figref{tcsag}), chose the guide star
    \begin{enumerate}
...
It is not possible to do it after a \gls{focus} sequence. 
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Go to he \texttt{FEROS Control} panel on \gls{desktop} \texttt{BOB+gen state} on screen \texttt{FEROS BOB}.
...
\begin{enumerate}
  ...
  \item Start guiding.
  \item \textbf{Important:}  at the end of the \gls{ob}, set the reference back to fibre position.
\end{enumerate}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Go to the workstation \texttt{WFI BOB}, \gls{desktop} \texttt{OS GUI}.
    ...
    \item Wait for filtre change ($\approx 1$ min)
      ...
      \end{itemize}
  \end{enumerate}
  \item Proceed with guiding as for a \gls{wfi} \gls{ob} (\secref{guidewfi}).
  ...
  In the \gls{TCS control panel}, click \texttt{Stop Monitoring} (under \texttt{AG Monitoring Parameters}) and click \texttt{Off} (in the panel \texttt{Autoguider}).  
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Edit the PRESET field to F\\
...
\item Deactivate \gls{feros} communication.\\ 
      In the FEROS control panel (\figref{feroscon}) select from menu \texttt{Telescope} $\rightarrow$ \texttt{IGNORE}.
...
\item Ensure \gls{feros} \gls{mirr3} mirror is not in the way.\\
      In panel FEROS Control (\figref{feroscon}), \texttt{\gls{M3} Selection Mirror} should state \texttt{WFI}\\
      If not, int the \gls{ics} control panel (\figref{ferosics}), check the \texttt{\gls{mirr3}} box, select \texttt{WFI}, click \texttt{SETUP}.
\item Ensure \gls{grond} \gls{M3} mirror is not in the way.\\
      In panel  GROND Control (\figref{grondcon}), \texttt{GROND \gls{M3}} should state \texttt{WFI}.\\
...
Then, go to \gls{bob} and use the menu \texttt{Configure} $\rightarrow$ \texttt{Environment}. In the \texttt{Process} tab change \gls{p2pp} by \gls{ot}. Now you can fetch the \gls{ob} from \gls{ot}.
...
It is possible to go on observing with GROND and FEROS while restarting the autoguider, provided that the modules are deactivated on the \gls{tcs}. However, you need to reactivate them to do the last step of restarting the autoguider (\texttt{e2p2Start\gls{TCCD}s}).
...
\begin{enumerate}
    ...
    \end{itemize}
    \item Remember to uncheck them for \texttt{e2p2Start\gls{TCCD}s} to work!
\end{enumerate}

\subsection{GROND}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see \procref{agmodules}.  You will need to uncheck them just before starting the guider software with \texttt{e2p2Start\gls{TCCD}s}.
...
\begin{enumerate}
   ...
   \begin{enumerate}
       ...
       \item Type \texttt{e2p2Stop\gls{TCCD}s gag} and wait for command to finish
       \item Type \texttt{e2p2Start\gls{TCCD}s gag} and wait for command to finish
   \end{enumerate}
   \item If you have time or \ref{list:gag1} does not solve the issue,
   go for the full restart (3--4 min)
   \begin{enumerate}
       \item On screen \texttt{Autoguider GROND \& FEROS}, find or open a terminal.
       \item Type \texttt{lccBoot \gls{l2p2agr}}
	   ...
	   \item Close the GUI
       \end{enumerate}
       \item Type \texttt{e2p2Start\gls{TCCD}s gag}
   \end{enumerate}
\end{enumerate}

\subsection{FEROS}
If you want to observe while fixing the issue you need to deactivate the modules in the \gls{tcs}, see \procref{agmodules}.  You will need to uncheck them before starting the guider software with \texttt{e2p2Start\gls{TCCD}s} (step~\ref{lab:fag2}). 
...
\begin{enumerate}
    \item Restart the \gls{ag} software (1--2 min).
        \begin{enumerate}
            ...
            \item Type \texttt{e2p2Stop\gls{TCCD}s fag}
            \item\label{lab:fag1} Type \texttt{e2p2Start\gls{TCCD}s fag}
    ...
    \item If the problem persists or an error pops up in step~\ref{lab:fag2}, reboot the \gls{ag} \gls{lcu} (3--4 min) 
        \begin{enumerate}
            \item Type \texttt{lccBoot \gls{l2p2cam}}
            \item Type \texttt{scanei} to open the scanei GUI
            \begin{enumerate}
...
\begin{enumerate}
  \item\label{wfiagrestart}
    ...
    \end{enumerate}
  \item Restart the WFI technical CCD.\\
    ...
    \begin{enumerate}
       ...
       \item Type \texttt{wfinsStop\gls{TCCD}S} and wait for it to complete
       \item Type \texttt{wfinsStart\gls{TCCD}S} and wait for it to complete (2 min)
      ...
      \item\label{list:wfifierarestart} Do a full restart of \gls{fiera} and \gls{wfi} (10-15~min).
        \begin{enumerate}
          ...
          \item Restart the CCD managing components
             \begin{enumerate} 
               \item Stop \glspl{TCCD} by typing \texttt{wfinsStopTCCDS}
               \item Stop \glspl{SCCD} by typing \texttt{wfinsStopSCCDS}
               \item Start SCCDs by typing \texttt{wfinsStartSCCDS}
               \item Start TCCDs by typing \texttt{wfinsStartTCCDS}
             \end{enumerate}
...
If the \gls{ob} crashes before the pop-up asking for manual preset, it means
bob must be killed and the \gls{sky flats} \gls{ob} restarted.
...
If the \gls{ob} crashes at the moment to change the filtre using \gls{wfi} and the \texttt{Filter Name} status is \texttt{Moving} on the \texttt{WFI \gls{General State}} panel (\figref{wfigen})
 \begin{enumerate}
      \item On the \texttt{e2p2 OS GUI} (\figref{wfios}), select the filtre on the \texttt{Filter} option below \texttt{Setup Instrument}.\label{list:filtchange1}
...
\begin{itemize}
   \item Quick workaround: each time that \gls{bob} displays a \gls{focus} order, enter it manually in the \gls{TCS control panel}, below \texttt{M2 Focus}, and click \texttt{Preset to}.  This focus bug will not impact science \glspl{ob} during the night, so don't worry.
...
\begin{itemize}
  \item Manually put it off from the \gls{ics} panel (\figref{ferosics})
     \begin{itemize}
  ...
  \item If in the last step there is an error that one of the devices that cannot be set
        \texttt{ONLINE}
      \begin{itemize}
       \item Use menu \texttt{\gls{lcu}} $\rightarrow$ \texttt{Reboot LCU1} 
...
\begin{itemize}
  \item There is an error about process \texttt{feoControl} stating
      ``accepted PING but did not reply properly within 10000 msec'''.\\
       Restart environment using \procref{restartenv}.
 \item There is a problem with the FEROS \gls{telemetry} at TCS (window should show some graphs, \figref{tcstelemetry}).\\
       Activate the telementry using \procref{telemetry}
\end{itemize}

\procedure{Restart environment}
...
\section{TCS issues}
\subsection{Start-up fails with \gls{l2p2cam} error}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{tcs/TCS-l2p2cam.jpg}
    \caption{When TCS restart fails to reboot the \gls{feros} \gls{ag} \gls{lcu}}
\end{figure}
...
\begin{enumerate}
    ...
    \item Do a remote reboot of the \gls{vme}.\\
          Type \texttt{lccBoot \gls{lte2p2}} in a terminal.
...
\begin{enumerate}
   ...
   \item On the dome controls (\figref{dome-control}) turn the control from local to remote
\end{enumerate}
...
\end{minipage}
\caption{Writing the \gls{M3} position in the database}
\label{fig:ccs-grondM3}
\end{figure}

\begin{figure}
\includegraphics[width=0.6\linewidth]{dome/dome-grond-m3-box.jpg}
\caption{The GROND \gls{M3} mirror and \gls{mc} control box.}
  ...
  \item Write the mirror position in the database (\figref{ccs-grondM3})
    \begin{enumerate}
      ...
      \item Look down \texttt{Appl\_data $\rightarrow$ GROND 
        $\rightarrow$ \gls{ics}  $\rightarrow$ DEVICES  $\rightarrow$ \gls{M3}}.
...
\begin{table}[!bt]
\centering
\caption{\Gls{pointing model} parameters for each instrument, as of November 2015.}
...
\gls{focus}, also used for \gls{feros}.}
\label{fig:wfistate}
\end{figure}
...
\texttt{\gls{mirr3}} from and back to \gls{wfi}.}
\label{fig:ferosics}
\end{figure}
...
\hline\hline
TCS \gls{lcu}/\gls{vme} & \texttt{lccBoot \gls{lte2p2}}  & \texttt{rsh vxte2p2 reboot}\\ 
...
\hline
FEROS LCU   & \texttt{lccBoot \gls{lfeics1}}       & FEROS \gls{ICS Control}, menu \texttt{LCU} $\rightarrow$ \texttt{Reboot}\\
