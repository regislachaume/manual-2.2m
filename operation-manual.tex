...
\newglossaryentry{fcdTelemetry}{name=\texttt{fcdTelemetry},sort=fcdTelemetry,description={Command to open the telemetry panel of GROND or WFI, used to activate telemetry if necessary.}}
\newglossaryentry{pickrefstar}{name=\texttt{Pick Reference Star},sort=Pick Reference Star,description={Graphical user interface that allows the observer to pick a guide star on the WFI and GROND autoguiders, and a fibre reference pixel for FEROS. Also a menu option to open such a window.}}
...
\newglossaryentry{osf2p2StartUp}{name=\texttt{osf2p2StartUp},sort=osf2p2StartUp,description={Command used to do a start-up of the telescope control software and the instruments (WFI \& FEROS).}}
\newglossaryentry{e2p2StopTCCDs}{name=\texttt{e2p2StopTCCDs},sort=e2p2StopTCCDs,description={Command to stop the technical CCDs (guiding cameras) of GROND and FEROS}}
\newglossaryentry{e2p2StartTCCDs}{name=\texttt{e2p2StartTCCDs},sort=e2p2StartTCCDs,description={Command to start the technical CCDs (guiding cameras) of GROND and FEROS}}
\newglossaryentry{grinsStop}{name=\texttt{grinsStop},sort=grinsStop,description={Command used to stop the GROND instrument}}
\newglossaryentry{grinsStart}{name=\texttt{grinsStart},sort=grinsStart,description={Command used to start the GROND instrument}}
...
\newglossaryentry{flip mirror}{name=flip mirror,description={Flip mirror, mirror in the infrared arm of GROND that enables fast dithering while optical is exposing. See grondFM}}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Set up the autoguider real time image.
      ...
      \item Activate the guide camera image flow if necessary.\\
        Use window menu \texttt{TCS} $\rightarrow$ \texttt{Attach camera}.
      \end{enumerate}
...
\begin{enumerate}
  ...
  \item In the \texttt{TCS} option, select \gls{pickrefstar}.
...
\begin{enumerate}
    ...
    \item If the problem persists or an error pops up in step~\ref{lab:fag2}, reboot the \gls{ag} \gls{lcu} (3--4 min) 
        \begin{enumerate}
            \item Type \texttt{\gls{lccBoot} \gls{l2p2cam}}
            \item Type \texttt{scanei} to open the scanei GUI
            \begin{enumerate}
  ...
  \item Restart the WFI technical CCD.\\
    ...
    \begin{enumerate}
       ...
       \item Stop the WFI \acrlong{tccd} with command
             \texttt{wfinsStopTCCDS} and wait for it to finish
       \item Start the WFI \acrlong{tccd} with command \texttt{wfinsStartTCCDS} and wait for it to complete (2 min)
      ...
      \item\label{list:wfifierarestart} Do a full restart of \gls{fiera} and \gls{wfi} (10-15~min).
        \begin{enumerate}
          ...
          \item Restart the CCD managing components
             \begin{enumerate} 
               \item Stop \acrlong{tccd} by typing \texttt{wfinsStopTCCDS}
               \item Stop \acrlong{sccd} by typing \texttt{wfinsStopSCCDS}
...
\caption{Local control of the \gls{dome}.  Local or remote control is set with the leftmost button.  Other two bottons on the same row set the direction and speed of rotation in the local control mode.  Just below, a start and stop button are used to initiate and finalise rotation.}
...
\hline
FEROS LCU   & \texttt{\gls{lccBoot} \gls{lfeics1}}       & FEROS \gls{ICS Control}, menu \texttt{LCU} $\rightarrow$ \texttt{Reboot}\\
