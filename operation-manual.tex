...
\begin{enumerate}
  \item Ensure that the error comes from WFI electronics configuration not being read
	\begin{enumerate}
		...
		\item Lines below and above contain ``C40 ping failed'' and ``Could not download FIERA config''
	\end{enumerate}
  \item\label{list:wfionline:reboot} Reboot the FIERA workstation
	\begin{itemize}
		\item In a terminal, type \texttt{rsh -l fcdrun wwffcdd reboot}
	        \item Wait for 1--2 minutes for it to get back online (you may check with \texttt{ping wwffcd})
	\end{itemize}
  \item Power cycle the WFI electronics 
	\begin{itemize}
		\item Go to the telescope floor
		\item Locate the WFI power unit close to the FIERA electronics
			(Fig.~\ref{fig:wfielectronics})
		\item Switch it off, wait for 30 secodns, then on.
	\end{itemize}
	\item\label{list:wfionline:restart} Do a full WFI restart
	\item If the error happens again, redo steps \ref{list:wfionline:reboot}--\ref{list:wfionline:restart}.
\end{enumerate}
